...
$(document).ready(function() {
  ...
  accordion_head.first().addClass("fa-rotate-135").next().show("normal"); accordion_head.on("click", function(event) {
    event.preventDefault();
    if (!$(this).hasClass("fa-rotate-135")) {
      accordion_body.hide("normal");
      $(this).next().stop(true, true).show("normal"); accordion_head.removeClass("fa-rotate-135");
...
$(document).ready(function() {
  ...
  function EasyTicker(f, g) {
    ...
    $(s.opts.controls.up).on("click", function(e) {
      e.preventDefault(); moveDir("up")
    });
    $(s.opts.controls.down).on("click", function(e) {
      e.preventDefault(); moveDir("down")
    });
    ...
    function init() {
      s.elem.children().css("margin", 0).children().css("margin", 0); s.elem.css({
        position: "relative",
    ...
    function move(a) {
      ...
      s.targ.stop(true, true).animate({
        ...
        c.hide()[appType](s.targ).fadeIn(); s.targ.css("top", 0); adjHeight()
      })
    }
    function moveDir(a) {
      stop();
    ...
    function fullHeight() {
      ...
      s.elem.css("display", "block"); s.targ.children().each(function() {
        a += $(this).outerHeight()
      }); s.elem.css({
        "display": b,
        "height": a
...
$(function() {
  ...
  Tab.prototype.activate = function(element, container, callback) {
    ...
    function next() {
      $active.removeClass("active").find("> .dropdown-menu > .active").removeClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded", false); element.addClass("active").find('[data-toggle="tab"]').attr("aria-expanded", true);
      if (transition) {
  ...
  $(document).ready(function() {
    ...
    function initPresets() {
      ...
      if (!items.length) {
        items = getPresets(); setStore(items)
      }
    }
    function getStoreItems() {
      ...
      items = JSON.parse(items); return items
    }
    function addToStore(item) {
      var items = getStoreItems();
      items.unshift(item); setStore(items)
    }
    ...
    function initLists() {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        li.innerHTML = '<a href="' + items[i].url + '" target="_blank" class="item" data-id="' + items[i].id + '" data-name="' + items[i].name + '"><img src="https://statics.dnspod.cn/proxy_favicon/_/favicon?domain=' + items[i].url.replace(/https:\/\/|http:\/\//ig, "") + '">' + items[i].name + '</a><i data-target="' + items[i].id + '" class="remove fa fa-times"></i>'; li.querySelector(".remove").addEventListener("click", onClickRemoveItem); li.querySelector("a.item").addEventListener("click", onClickItem); myList.appendChild(li)
      }
      sortable = createSort()
    }
    function onClickItem(e) {
      ...
      addRecentItem(item); initRecentItems()
    }
    function onClickRemoveItem(e) {
      e.preventDefault(); var targetID = this.getAttribute("data-target");
      var items = getStoreItems();
      for (var i = 0; i < items.length; i++) {
      ...
      setStore(items); sortable.destroy();
      myList.innerHTML = ""; initLists()
    }
    ...
    $(".btn-close-fm").on("click", function() {
      ...
      myList.innerHTML = ""; initLists(); this.reset(); this.querySelector("input").focus()
    });
    function createSort() {
    ...
    function addRecentItem(item) {
      ...
      items.unshift(item); setRecentItems(items)
    }
    function initRecentItems() {
      ...
      for (var i = 0; i < items.length; i++) {
        var li = document.createElement("li");
        var url = items[i].url;
        if (url.lastIndexOf("index.html") === url.length - 1) {
          url = url.substring(0, url.length - 1)
        }
        li.setAttribute("data-id", items[i].name);
        li.innerHTML = '<a href="' + url + '" target="_blank" class="item" data-name="' + items[i].name + '"><img src="http://statics.dnspod.cn/proxy_favicon/_/favicon?domain=' + url.replace(/https:\/\/|http:\/\//ig, "") + '">' + items[i].name + '</a><i data-target="' + items[i].name + '"class="remove fa fa-times"></i>'; li.querySelector(".remove").addEventListener("click", onClickRemoveRecentItem); li.querySelector("a.item").addEventListener("click", onClickItem);
    ...
    function onClickRemoveRecentItem(e) {
      ...
      setRecentItems(items); initRecentItems()
    }
    initRecentItems();
